...
module.exports = function (backendBranchName) {
  ...
  if (backendBranchRemote === '') {
    ...
    execSync(`git pull`, { cwd: backendDeployDir, stdio: [0, 1, 2] })
  }
  ...
  // 同步前端代码到 S3
  execSync(`aws s3 sync ${FRONTEND_DEPLOY_REL_PATH} s3://easi-assets/${frontProjectName}/`)
  ...
  if (backendBranchStatusLocal !== '') {
    ...
    execSync(
      `git commit -m "chore(${frontProjectName}): build(#${frontendRev})"`,
      { cwd: backendDeployDir, stdio: [0, 1, 2] }
    )
    execSync(`git push`, { cwd: backendDeployDir, stdio: [0, 1, 2] })
  }
