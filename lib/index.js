...
module.exports = function (env, backendBranchName) {
  ...
  if (!fs.existsSync(backendDeployDir)) {
    ...
    console.log(`git clone ${backendRepositoryUrl} ${backendDeployDir}`)
    execSync(
  ...
  console.log(`git ls-remote --heads ${backendRepositoryUrl} ${backendBranchName}`)
  ...
  if (backendBranchRemote === '') {
    ...
    console.log(`git fetch origin ${backendBranchName}`)
  ...
  console.log(`git branch --list ${backendBranchName}`)
  if (backendBranchLocal === '') {
    // 如果后端不存在本地对应分支，直接创建
    console.log(`git branch --no-track ${backendBranchName} refs/remotes/origin/${backendBranchName}`)
    execSync(
      `git branch --no-track ${backendBranchName} refs/remotes/origin/${backendBranchName}`,
      { cwd: backendDeployDir, stdio: [0, 1, 2] },
    )
    console.log(`git branch --set-upstream-to=origin/${backendBranchName} ${backendBranchName}`)
    execSync(
      `git branch --set-upstream-to=origin/${backendBranchName} ${backendBranchName}`,
      { cwd: backendDeployDir, stdio: [0, 1, 2] },
    )
  }
  console.log(`git checkout ${backendBranchName}`)
