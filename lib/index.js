...
module.exports = function (env, backendBranchName) {
  ...
  const publicPath = PUBLIC_PATHS[env]
  if (!publicPath) {
    throw Error(`easi-assets-deploy: public path not found for ${env}.`)
  }
  ...
  execSync(`EASI_ASSETS_PUBLIC_PATH=${publicPath} yarn build`, frontendExecOptions)
  // 复制 dist 代码
